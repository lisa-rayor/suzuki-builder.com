...
for (const name of names) assert(fs.existsSync(path.join(root, `${name}.html`)), `Missing page: ${name}.html`);
...
try {
  ...
  const snapshot = async page => page.evaluate(() => {
    const visible = [...document.body.querySelectorAll('*')].filter(el => !['SCRIPT', 'STYLE'].includes(el.tagName));
    return visible.map(el => {
      const rect = el.getBoundingClientRect(), style = getComputedStyle(el);
      return { tag: el.tagName, text: el.children.length ? '' : el.textContent.replace(/\s+/g, ' ').trim(), box: [rect.x, rect.y, rect.width, rect.height], styles: [...style].map(key => [key, style.getPropertyValue(key).replaceAll('/before/', '/')]) };
    });
  });
  ...
  for (const width of [1440, 390]) for (const name of names) {
    ...
    for (const [i, page] of pages.entries()) {
      ...
      await page.goto(`${origin}/${i ? `${name}.html` : `before/${name}.php`}`, { waitUntil: 'networkidle' });
      await page.evaluate(async () => { await document.fonts.ready; await Promise.all([...document.images].map(image => image.decode())); });
    }
    assert.deepEqual(errors, [], `${name}/${width}: JavaScript errors`);
    const cssRules = page => page.evaluate(() => [...[...document.styleSheets].find(sheet => sheet.href?.endsWith('/css/creative.css')).cssRules].map(rule => rule.cssText));
    ...
    const links = await pages[1].locator('a[href]').evaluateAll(nodes => nodes.map(node => node.getAttribute('href')).filter(href => /^[\w-]+\.html$/.test(href)));
    for (const link of links) assert(fs.existsSync(path.join(root, link)), `Broken local link: ${link}`);
    if (name === 'contact') {
